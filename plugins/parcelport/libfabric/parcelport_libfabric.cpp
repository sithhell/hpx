...
namespace libfabric
{
    ...
    bool parcelport::do_run()
    {
        ...
        {
            bool res = senders_.bounded_push(
                new sender(this,
            ...
            if (!res)
            {
                throw std::runtime_error("Could not add sender");
            }
        }
...
//                parcelset::locality(locality(buf.s_addr));
    ...
    bool parcelport::background_work(std::size_t num_thread) {
        ...
        while (result)
        {
            result = libfabric_controller_->poll_endpoints();

            if (dequeue_parcel(dest, p, f))
            {
                send_immediate_impl<parcelport>(
                    *this, dest, std::move(f), std::move(p));
                result = true;
            }
        }

        // always pretend we did work...
        return true;
    }
}}}}
