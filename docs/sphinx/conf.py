...
hpx_source_dir = os.path.abspath('../..')

# Make our custom HPX extension visible.
sys.path.insert(0, os.path.abspath('.') + '/extensions')

from generate_breathe_projects import generate_breathe_projects
from parse_cmake import generate_cmake_doc
...
breathe_projects_source = generate_breathe_projects(hpx_source_dir)

# parse the CMakeLists.txt file and auto generate the documentation
# for the toolchain files as well as the CMake Options
hpx_version, hpx_version_full = generate_cmake_doc(hpx_source_dir)
...
breathe_doxygen_config_options = {
    ...
    'MACRO_EXPANSION': 'YES',
    'DIRECTORY_GRAPH': 'NO',
    'PREDEFINED': ' '.join(predefined_macros),
...
try:
    repo = git.Repo(hpx_source_dir)
    html_context['commit'] = repo.head.object.hexsha[:11]
